...
from app.db.schemas import StockQuote, StockHistoricalData, StockPrediction
...
async def get_stock_quote(
    ...
    if not quote_data:
        raise HTTPException(
            status_code=404,
            detail=f"Stock quote not found for symbol: {symbol}"
        )
...
async def get_popular_stocks(
    ...
    return {
        "stocks": [StockQuote(**quote) for quote in quotes]
    }
